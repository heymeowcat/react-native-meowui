...
// --- List.Icon ---
export const ListIcon = ({ icon, color, size = 24 }: { icon: any, color?: string, size?: number }) => {
   const { theme } = useTheme();
   // Fallback to text if icon is string to avoid vector-icon dependency
   return (
     <View style={styles.iconContainer}>
        {typeof icon === 'string' && icon.length < 3 ? (
             <Text style={{ fontSize: size, color: color || theme.colors.onSurface }}>{icon}</Text>
        ) : (
...
export interface ListItemProps {
  ...
  right?: (props: { color: string, style: object }) => React.ReactNode;
  onPress?: () => void;
  ...
  return (
    ...
    >
      ...
      <View style={styles.content}>
        ...
        {description && (
          <Text style={[styles.description, { color: theme.colors.onSurfaceVariant }, descriptionStyle]}>
            {description}
          </Text>
        )}
      </View>
...
// --- List.Section ---
export const ListSection = ({ title, children, style }: { title?: string, children: React.ReactNode, style?: StyleProp<ViewStyle> }) => {
  const { theme } = useTheme();
  return (
...
export const ListAccordion = ({
  ...
  id,
  style,
}: ListItemProps & { expanded?: boolean, children: React.ReactNode, id?: string | number }) => {
  const { theme } = useTheme();
