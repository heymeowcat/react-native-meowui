...
export const Checkbox = ({ status, onPress, disabled = false, color, uncheckedColor, size = 24, style, }) => {
    const { theme } = useTheme();
    const progress = useSharedValue(status === 'checked' || status === 'indeterminate' ? 1 : 0);
    const scale = useSharedValue(1);
    useEffect(() => {
        progress.value = withSpring(status === 'checked' || status === 'indeterminate' ? 1 : 0, theme.animations.springs.bouncy);
    }, [status, theme]);
    const handlePressIn = () => {
    ...
    return (<Pressable onPress={onPress} onPressIn={handlePressIn} onPressOut={handlePressOut} disabled={disabled} hitSlop={8} style={[styles.container, style]}>
      <Animated.View style={[
            styles.box,
            {
                width: size,
