...
export const Combobox = ({ label, value, items, onValueChange, placeholder = 'Select item...', disabled = false, style, }) => {
    ...
    return (<View style={style}>
      {label && (<Text style={[styles.label, { color: theme.colors.onSurfaceVariant }]}>
          {label}
        </Text>)}
      
      <Pressable ref={triggerRef} onPress={openDropdown} style={({ pressed }) => [
            styles.trigger,
            {
      ...
      {modalVisible && (<Portal>
        <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
          ...
          <Animated.View entering={FadeInUp.springify().damping(30).mass(0.8).stiffness(250)} exiting={FadeOutUp.duration(200)} style={[
                styles.dropdown,
                {
                    top: layout.y,
                    left: layout.x,
             ...
             <View style={styles.header}>
               <Searchbar value={searchQuery} onChangeText={setSearchQuery} placeholder="Search..." style={{ height: 40, borderWidth: 0, shadowOpacity: 0, elevation: 0 }}/>
             </View>
             
             <ScrollView style={styles.list} keyboardShouldPersistTaps="handled">
               {filteredItems.map((item) => (<Pressable key={item.value} onPress={() => handleSelect(item.value)} style={({ pressed }) => [
                    styles.item,
                    {
                   ...
                   {item.value === value && (<Text style={{ color: theme.colors.primary, fontWeight: '900' }}>✓</Text>)}
                 </Pressable>))}
               {filteredItems.length === 0 && (<View style={styles.empty}>
