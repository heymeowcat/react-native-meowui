...
export const RadioButton = ({ status, onPress, disabled = false, color, uncheckedColor, size = 24, style, }) => {
    const { theme } = useTheme();
    const progress = useSharedValue(status === 'checked' ? 1 : 0);
    const scale = useSharedValue(1);
    useEffect(() => {
        progress.value = withSpring(status === 'checked' ? 1 : 0, theme.animations.springs.bouncy);
    }, [status, theme]);
    const handlePressIn = () => {
