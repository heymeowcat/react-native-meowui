...
export const Input = ({ label, variant = 'outlined', size = 'medium', error = false, errorMessage, helperText, leftIcon, rightIcon, gradientFocus = true, style, inputStyle, value, onFocus, onBlur, placeholder, ...textInputProps }) => {
    ...
    const animatedLabelStyle = useAnimatedStyle(() => {
        const fontSize = interpolate(focusProgress.value, [0, 1], [14, 12]);
        const translateY = interpolate(focusProgress.value, [0, 1], [0, -sizeStyles.height / 2 - 4]);
        return {
            fontSize,
    ...
    const animatedContainerStyle = useAnimatedStyle(() => {
        const borderColor = interpolateColor(borderProgress.value, [0, 1], [
    ...
    return (<View style={[styles.container, style]}>
      ...
      <Animated.View style={[getVariantStyles(), animatedContainerStyle]}>
        <Pressable onPress={handleContainerPress} style={styles.inputContainer}>
          ...
          <TextInput ref={inputRef} value={value} placeholder={placeholder} placeholderTextColor={theme.colors.onSurfaceVariant} onFocus={handleFocus} onBlur={handleBlur} style={[
            styles.input,
            {
